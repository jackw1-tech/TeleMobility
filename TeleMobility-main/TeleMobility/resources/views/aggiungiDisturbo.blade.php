...
<div class="container">
    ...
    @if($disturbiDisponibili)

        <div style="text-align:center; margin-top:60px;margin-bottom:60px">
            <h1>Nessun disturbo selezionabile</h1>
        </div>
        
    @else
    
   <h2>Elenco dei disturbi selezionabili</h2>
...
@endif
   @foreach ($disturbi as $disturbo => $nome)
        {{ html()->form('POST', route('disturbo.storeClinico', ['idPaziente' => $idPaziente]) )->acceptsFiles()->open() }}
        <div class="containerEvento" style="margin-top:20px">
                <div class="event-infoEvento"> 
                        <h4> {{$nome}} </h4>
                </div>
                {{ html()->hidden('ID_Disturbo', $disturbo) }}
                {{ html()->button('Aggiungi Disturbo')->type('submit')->name('submitDisturbo_' . $disturbo)->class('submitDisturbo')}}
        </div>
        {{ html()->form()->close() }}
   @endforeach
   @endif
